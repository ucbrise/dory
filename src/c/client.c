...
int getIndexesForKeyword(client *c, uint32_t indexes[], char *keyword) {
    ...
    uint8_t modValue = (base == ceil((double)BLOOM_FILTER_SZ / BLOCK_SZ) - 1) ? (BLOOM_FILTER_SZ % BLOCK_SZ == 0 ? BLOCK_SZ : BLOOM_FILTER_SZ % BLOCK_SZ) : min(BLOOM_FILTER_SZ, BLOCK_SZ);
