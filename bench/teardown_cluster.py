...
cmd = ('export AWS_DEFAULT_REGION=us-east-2; aws ec2 terminate-instances --instance-ids "%s"') % (sysConfig["BaselineServerID"])
print(cmd)
process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
process.wait()
...
cmd = ('export AWS_DEFAULT_REGION=us-east-1; aws ec2 terminate-instances --instance-ids "%s"') % (sysConfig["BaselineClientID"])
print(cmd)
process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
process.wait()
