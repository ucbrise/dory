...
def runExperimentRound(numClusters):
    ...
    f91 = open("out/dory_semihonest_throughput_" + str(numClusters) + "_9_1.dat", "w")
    f55 = open("out/dory_semihonest_throughput_" + str(numClusters) + "_5_5.dat", "w")
    f19 = open("out/dory_semihonest_throughput_" + str(numClusters) + "_1_9.dat", "w")
    ...
    for i in range(11):
        ...
        servers = initForDoryMixedThroughput(bloomFilterSz, numDocs, tickMs, clientS, threads, numClusters, isMalicious)

        numUpdates = 9
        numSearches = 1
        print(("9/1 for %d clusters, %d docs") % (numClusters, numDocs))
        throughput = runDoryMixedThroughputTest(bloomFilterSz, numDocs, tickMs, clientS, threads, numUpdates, numSearches, numClusters, isMalicious, isLeaky, useMaster)
        print("-------------------------")
